...
<template>
  <section class="schedule-page">
    ...
    <template v-if="schedule">
      ...
      >
        ...
        <button
          ...
          :class="{ 'meeting-card-short': segment.startPeriod === segment.endPeriod, 'meeting-card-two-periods': segment.endPeriod - segment.startPeriod === 1, 'meeting-card-conflict': segment.candidates.length > 1, 'meeting-card-other-week': segment.otherWeek }"
          :style="{
            '--course-color': normalizeScheduleColor(segment.item.course.color),
    ...
    <Transition name="sheet"><section v-if="selected" class="course-detail" role="dialog" aria-modal="true" :aria-label="selected.course.name">
      <div class="sheet-handle" /><header><i :style="{ background: normalizeScheduleColor(selected.course.color) }" /><h2>{{ selected.course.name }}</h2><button aria-label="关闭课程详情" @click="selected = null"><X /></button></header>
...
.course-detail{max-height:calc(100dvh - var(--safe-area-inset-top) - 24px);overflow-y:auto;overscroll-behavior:contain}
