...
<template>
  <section class="schedule-page">
    <header class="schedule-header">
      <div><span class="schedule-kicker">{{ schedule?.name || 'LUMATILE SCHEDULE' }}</span><div class="schedule-title-row"><h1>{{ title }}</h1><button v-if="schedule && week !== Math.max(1, Math.min(schedule.totalWeeks, weekForDate(schedule)))" class="current-week-button" @click="currentWeek">回到本周</button></div><p>{{ dateRange || '把教务课表装进口袋' }}</p></div>
