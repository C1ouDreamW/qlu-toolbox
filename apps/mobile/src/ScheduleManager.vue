  ...
  <section class="manager-page">
    ...
    <main v-else>
      ...
      <section class="settings-group">
        ...
        <div v-for="(item, index) in draft.noClassDates" :key="item.date" class="off-row"><span><strong>{{ item.date }}</strong><small>{{ item.reason }}</small></span><button @click="removeNoClassDate(index)"><Trash2 /></button></div>
      </section>
      <section class="settings-group">
        <h2><Clock3 />上课时间</h2>
        <div class="period-list"><label v-for="period in draft.periods" :key="period.period"><strong>{{ period.period }}</strong><input v-model="period.start" type="time" @change="save" /><span>—</span><input v-model="period.end" type="time" @change="save" /></label></div>
      </section>
