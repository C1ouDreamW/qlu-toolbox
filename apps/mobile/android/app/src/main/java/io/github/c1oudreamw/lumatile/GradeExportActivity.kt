...
class GradeExportActivity : AppCompatActivity() {
    ...
    private val gradeExportScript by lazy { assets.open("grade-export.js").bufferedReader().use { it.readText() } }
    ...
    private fun buildExportScript(): String {
        val config = JSONObject()
            ...
            .put("exportUrl", EXPORT_URL)
            .put("columns", JSONArray(EXPORT_COLUMNS))
            .put("maxFileSize", MAX_FILE_SIZE)
        return "window.__LUMATILE_GRADE_EXPORT_CONFIG__ = $config;\n$gradeExportScript"
    }
