...
import { Search, FileSpreadsheet, Calculator, BookOpenCheck, ArrowUpRight, ShieldCheck, Sparkles } from 'lucide-vue-next'
...
const pageFor = (tool: ToolManifest): PageName => tool.id === 'gpa-calculator' ? 'gpa' : tool.id === 'credit-report' ? 'credit' : 'grade'
</script>
    ...
    <div class="tool-grid">
      <article v-for="tool in tools" :key="tool.id" class="tool-card featured" @click="emit('navigate', pageFor(tool))">
        <div class="tool-card-top">
          <div class="tool-icon large excel"><Calculator v-if="tool.id === 'gpa-calculator'" :size="25" /><BookOpenCheck v-else-if="tool.id === 'credit-report'" :size="25" /><FileSpreadsheet v-else :size="25" /></div>
          <span class="tool-version">v{{ tool.version }}</span>
        </div>
        <span class="category">{{ tool.category }}</span><h2>{{ tool.name }}</h2><p>{{ tool.description }}</p>
        <div class="tool-meta"><span><ShieldCheck :size="15" /> 本地处理</span><span><Sparkles :size="15" /> {{ tool.id === 'gpa-calculator' ? '动态计算' : tool.id === 'credit-report' ? '自动核对' : '自动校验' }}</span></div>
