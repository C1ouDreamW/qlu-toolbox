...
import { Search, FileSpreadsheet, Calculator, CalendarDays, BookOpenCheck, ArrowUpRight, ShieldCheck, Sparkles } from 'lucide-vue-next'
...
const pageFor = (tool: ToolManifest): PageName =>
  tool.id === 'gpa-calculator' ? 'gpa' : tool.id === 'schedule' ? 'schedule' : tool.id === 'credit-report' ? 'credit' : 'grade'
</script>
      ...
      <article v-for="tool in tools" :key="tool.id" class="tool-card featured" @click="emit('navigate', pageFor(tool))">
        <div class="tool-card-top">
          <div class="tool-icon large excel">
            ...
            <BookOpenCheck v-else-if="tool.id === 'credit-report'" :size="25" />
