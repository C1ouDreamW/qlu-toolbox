...
// 桌面端循环滚轮选择器：拖拽/滚轮/方向键/点选均可换挡，逻辑与移动端共享 @lumatile/schedule-ui。
...
function onPointerDown(event: PointerEvent) {
  ...
  try { rootEl.value?.setPointerCapture(event.pointerId) } catch { /* 合成事件等场景无有效指针 */ }
}
...
<style scoped>
.wheel{position:relative;height:190px;overflow:hidden;touch-action:none;user-select:none;-webkit-user-select:none;outline:none}
.wheel-track{will-change:transform}
.wheel-track.settling{transition:transform .22s cubic-bezier(.2,.75,.3,1)}
.wheel-item{display:flex;height:38px;flex-direction:column;align-items:center;justify-content:center;gap:1px}
.wheel-item .wheel-label{font-size:var(--font-body);font-weight:600;color:var(--muted)}
.wheel-item .wheel-sub{font-size:10px;font-weight:600;color:var(--faint)}
.wheel-item.active .wheel-label{color:var(--primary);font-weight:700}
.wheel-band{position:absolute;top:50%;right:8px;left:8px;height:38px;transform:translateY(-50%);border-top:1px solid var(--line);border-bottom:1px solid var(--line);border-radius:9px;background:color-mix(in srgb,var(--primary) 5%,transparent);pointer-events:none}
.wheel-mask{position:absolute;right:0;left:0;height:62px;pointer-events:none}
.wheel-mask-top{top:0;background:linear-gradient(180deg,var(--surface) 12%,rgba(0,0,0,0))}
.wheel-mask-bottom{bottom:0;background:linear-gradient(0deg,var(--surface) 12%,rgba(0,0,0,0))}
.wheel-disabled{opacity:.35;pointer-events:none}
.wheel:focus-visible .wheel-band{outline:2px solid color-mix(in srgb,var(--primary) 35%,transparent);outline-offset:2px}
