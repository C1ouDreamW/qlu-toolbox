...
| 平台 | 当前版本 | 发布形式 | 说明 |
|---|---:|---|---|
| Windows x64 | 2.0.4 | NSIS 安装包、免安装 ZIP | 支持 Edge、Chrome 和按需下载的 Chromium |
| macOS Apple Silicon | 2.0.4 | DMG | 未签名、未公证，首次打开需在系统设置中确认 |
| Android 7.0 及以上 | 2.0.4（versionCode 15） | APK | 使用系统 WebView（Chromium 80 及以上），支持系统文件保存和应用内更新检查 |
...
- **学分修读情况（v2.0.1 新增）**：手动登录后对照培养方案统计综合素质选修课的已修、在修学分，展示模块缺口与选课建议，支持专业类型选择和本地学分要求调整。
...
- 启动时更新检查只读取公开更新源上的静态清单。「匿名使用统计」开启时，客户端每次启动额外向同一服务器发送一次心跳，内容仅为随机安装编号、软件版本和系统类型，不含任何个人信息，也不与账号或成绩关联；该功能与"启动时检查更新"是两个独立开关，可分别关闭。
- 应用每次启动还会读取一次更新源公告（对静态文件的普通 GET 请求，不带任何标识符），同一公告只弹窗一次。
