...
if(m==='saveSchedule'){if(window.failSave)throw Error('模拟磁盘写入失败');await new Promise(r=>setTimeout(r,120));auditBoot.schedules=[{...p,isActive:true,updatedAt:new Date().toISOString()}];return p;}}};''')
                ...
                page.get_by_role('button',name='导入展示课表',exact=True).click()
                page.get_by_role('heading',name='导入预览',exact=True).wait_for()
                self.assertIn('全场景展示课表（虚拟）', page.locator('.import-name').inner_text())
                page.locator('.modal-close').click()
                page.get_by_role('combobox',name='选择周次').click()
                page.get_by_role('option',name='第 3 周').click()
                self.assertEqual(page.get_by_role('combobox',name='选择周次').inner_text(), '第 3 周')
                ...
                page.get_by_role('combobox',name='选择周末显示方式').click()
                page.get_by_role('option',name='有课时显示').click()
                page.wait_for_function("JSON.parse(auditBoot.schedules[0].payload).weekendMode==='auto'")
                ...
                page.get_by_role('heading',name='导入预览',exact=True).wait_for()
                page.get_by_role('button',name='覆盖已有课表',exact=True).click()
                page.get_by_role('combobox',name='选择覆盖目标').click()
                page.get_by_role('option',name='改名').click()
                self.assertEqual(page.get_by_role('combobox',name='选择覆盖目标').inner_text(), '改名')
                ...
                mobile.get_by_role('button',name='导入课表',exact=True).click()
                mobile.get_by_role('button',name='导入展示课表',exact=False).click()
                self.assertEqual(mobile.locator('.import-dialog h2').inner_text(), '全场景展示课表（虚拟）')
                mobile.locator('.import-dialog header button').click()
                ...
                book['courses'].append(dict(id='c3', name='单节课程', teachers=[], color='#336699', credit=None, code='', teachingClass='', note='',
                    meetings=[dict(id='m3', weeks=list(range(1,20)), weekday=2, startPeriod=3, endPeriod=3,
                                   location='彩石南314315', teachers=[], source='manual')]))
                ...
                self.assertIn('@彩石北322323', card_text)
                self.assertNotIn('重点章节', card_text)
                self.assertNotIn('管红娇', card_text)
                self.assertEqual(mobile.locator('.course-location').first.evaluate('(e)=>getComputedStyle(e).webkitLineClamp'), '5')
                self.assertIn('@彩石南314315', mobile.get_by_role('button', name='单节课程', exact=False).first.inner_text())
                self.assertEqual(mobile.get_by_role('button', name='单节课程', exact=False).first.locator('.course-location').evaluate('(e)=>getComputedStyle(e).webkitLineClamp'), '1')
                mobile.get_by_role('button', name='2项重叠安排', exact=False).first.click()
                self.assertIn('重点章节', mobile.locator('.course-detail').inner_text())
                self.assertIn('管红娇', mobile.locator('.course-detail').inner_text())
                ...
                self.assertIn('@彩石北322323', desktop_text)
                self.assertNotIn('重点章节', desktop_text)
                self.assertNotIn('管红娇', desktop_text)
